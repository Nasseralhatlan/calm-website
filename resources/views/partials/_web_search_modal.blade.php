...
<div x-data="calmSearchModal(@js($searchModalCatalog), { redirect: @js((bool) ($searchModalRedirect ?? true)) })"
    ...
    <div class="mx-auto w-full" style="max-width: 560px; padding: 16px 20px 0; flex: 1 1 0%; min-height: 0; display: flex; flex-direction: column;">
        ...
        <div style="flex: 1 1 0%; min-height: 0; display: flex; flex-direction: column; gap: 12px; padding-bottom: 100px;">
            ...
            {{-- ── وين ؟ ── --}}
            <div class="bg-white flex flex-col" :style="active === 'where' ? {!! $expandedFlex !!} : {!! $collapsedFlex !!}">
            ...
            {{-- ── متى ؟ ── --}}
            <div class="bg-white flex flex-col" :style="active === 'when' ? {!! $expandedFlex !!} : {!! $collapsedFlex !!}">
                    ...
                    <div class="shrink-0 grid grid-cols-3" style="gap: 8px; margin-top: 12px;">
                        <template x-for="ch in quickChips()" :key="ch.iso">
                            <button type="button" @click="sel.checkIn = ch.iso; sel.checkOut = ch.iso"
                                ...
                                <span class="block font-bold text-black" style="font-size: 11px;" x-text="ch.label"></span>
                                <span class="block" dir="ltr" style="font-size: 10px; color: #AAAAAA; margin-top: 1px;" x-text="ch.sub"></span>
                            </button>
                        </template>
                    </div>
            ...
            {{-- ── نوع المكان ؟ ── --}}
            <div class="bg-white flex flex-col" :style="active === 'type' ? {!! $expandedFlex !!} : {!! $collapsedFlex !!}">
            ...
            <div class="bg-white flex flex-col" x-show="cityAreas().length > 0"
                 :style="active === 'area' ? {!! $expandedFlex !!} : {!! $collapsedFlex !!}">
    ...
    <div class="fixed inset-x-0 bottom-0" style="padding: 14px 20px calc(18px + env(safe-area-inset-bottom, 0px));">
        <div class="mx-auto flex items-center justify-between" style="max-width: 560px;">
            ...
            <button type="button" @click="apply()"
                    ...
                    style="padding: 15px 30px; border-radius: 18px; gap: 9px; font-size: 15px; background-color: #F88379; box-shadow: 0 6px 12px rgba(248,131,121,0.3);">
    ...
    if (!window.calmSearchModal) {
        window.calmSearchModal = function (catalog, opts) {
            ...
            return {
                ...
                pickDay(iso) {
                    // Some browsers double-fire the tap — ignore an identical
                    // pick arriving within 400ms so the range isn't restarted.
                    const now = Date.now();
                    if (this._lastPick && this._lastPick.iso === iso && now - this._lastPick.t < 400) return;
