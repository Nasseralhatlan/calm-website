...
@section('main')
    {{-- ── Summary cards ── --}}
    <div class="grid grid-cols-1 sm:grid-cols-3" style="gap: 14px; margin-bottom: 18px;">
    ...
    <form method="GET" action="{{ route('admin.payouts.index') }}" class="flex items-center bg-white {{ $fa }}"
        ...
        @if($tab === 'paid')<input type="hidden" name="tab" value="paid">@endif
        ...
        @if($search)
            <a href="{{ route('admin.payouts.index', array_filter(['tab' => $tab === 'paid' ? 'paid' : null])) }}" class="text-[13px] text-[#717171] hover:text-[#222]" style="padding: 0 12px;">✕</a>
        @endif
    </form>

    <div class="flex items-center flex-wrap" style="gap: 8px; margin-bottom: 18px;">
        @foreach(['pending' => $isRtl ? 'بانتظار التحويل' : 'Pending', 'paid' => $isRtl ? 'المدفوعة' : 'Paid'] as $key => $label)
            @php
                $active = $tab === $key;
                $tint = $key === 'pending' ? '#f59e0b' : '#10b981';
                $count = $key === 'pending' ? $totals['pending_count'] : $totals['paid_count'];
            @endphp
            <a href="{{ route('admin.payouts.index', array_filter(['q' => $search, 'tab' => $key === 'paid' ? 'paid' : null])) }}"
