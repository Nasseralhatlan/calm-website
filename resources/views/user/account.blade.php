...
<div class="min-h-screen" style="background-color: #FBFBFB;">
    ...
    <main class="mx-auto w-full" style="max-width: 720px; padding: 36px 20px 170px;">
        <h1 class="font-bold text-black {{ $fa }}" style="font-size: 28px; line-height: 34px;">{{ $isRtl ? 'حسـابـى' : 'Account' }}</h1>

        @if(! $me)
            @include('partials._web_login_prompt', [
                'promptTitle' => $isRtl ? 'سجّل دخولك' : 'Sign in',
                'promptSubtitle' => $isRtl ? 'سجّل دخولك لإدارة حسابك وحجوزاتك.' : 'Sign in to manage your account and bookings.',
            ])
        @else
            {{-- Identity card --}}
