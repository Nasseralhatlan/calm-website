...
it('creates a pending booking and returns a Moyasar payment url', function (): void {
    ...
    // The Moyasar invoice carries the FULL identity set — searchable in the
    // dashboard, echoed in webhooks. (Form-encoded as metadata[key] pairs.)
    Http::assertSent(fn ($request): bool => str_ends_with($request->url(), '/invoices')
        ...
        && data_get($request->data(), 'metadata[guest_id]') === $booking->guest_user_id
        && data_get($request->data(), 'metadata[host_id]') === $booking->host_user_id);
});
...
it('confirms instantly with a mock payment when BOTH parties are review accounts', function (): void {
    config()->set('sms.mock_phones', ['0500000001', '0500000002']);
    Http::fake();

    $demoHost = User::factory()->create(['phone' => '500000002']);
    $demoGuest = User::factory()->create(['phone' => '500000001']);
    $place = bookingPlace(['host_user_id' => $demoHost->id]);
    [$in, $out] = twoNightDates();

    $this->actingAs($demoGuest, 'api')
        ->postJson("/api/places/{$place->id}/bookings", ['check_in' => $in, 'check_out' => $out, 'guests' => 2])
        ->assertStatus(201)
        ->assertJsonPath('data.status', 'confirmed');
    ...
    expect($booking->booking_status)->toBe(BookingStatus::Confirmed)
        ...
        ->and($booking->payment_method)->toBe('mock')
        ->and($booking->payment_id)->toStartWith('mock_')
        ->and($booking->payment_url)->toBeNull()
        ->and($booking->expires_at)->toBeNull();

    // No Moyasar call, and no real-money movement recorded.
    Http::assertNothingSent();
    expect($booking->financialMovements()->count())->toBe(0);
});

it('uses the REAL payment flow when a review guest books a real host', function (): void {
    config()->set('sms.mock_phones', ['0500000001']);
    fakeMoyasar();

    $demoGuest = User::factory()->create(['phone' => '500000001']);
    $place = bookingPlace(); // real (non-mock) host
    [$in, $out] = twoNightDates();
    ...
    // A real Moyasar invoice was created — real hosts are never bookable for free.
    Http::assertSent(fn ($request): bool => str_ends_with($request->url(), '/invoices'));
});
