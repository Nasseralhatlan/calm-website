...
it('never invoices, credits, or pays out a demo (mock-payment) booking', function (): void {
    Http::fake();
    $booking = fdocBooking(fdocPlace($this->host), $this->guest, [
        'payment_method' => 'mock',
        'payment_id' => 'mock_review1',
        'booking_status' => BookingStatus::Completed->value,
    ]);

    // The sweep skips it entirely — no documents, no movements, no Qoyod.
    (new FinalizeBookingFinances)->handle(app(BookingFinanceFinalizer::class), app(QoyodSyncService::class));
    expect($booking->refresh()->financial_completed_at)->toBeNull()
        ->and($booking->financialDocuments()->count())->toBe(0)
        ->and($booking->financialMovements()->count())->toBe(0);

    // Cancellation makes no refund call and mints no cash-leg documents.
    $booking->update(['booking_status' => BookingStatus::Confirmed->value]);
    app(BookingService::class)->cancelByAdmin($booking->refresh(), BookingStatus::CanceledByAdmin);
